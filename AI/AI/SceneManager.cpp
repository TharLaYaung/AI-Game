...
void SceneManager::Draw() {
    ...
    float scale = 1.0f + 0.1f * sin(time * 0.005f);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        int px1 = mouseX + (int)(cos(a) * radius);
        int py1 = mouseY + (int)(sin(a) * radius);
        float a_next = angle1 + (i + 1) * (3.14159f * 2.0f / 3.0f);
        int px2 = mouseX + (int)(cos(a_next) * radius);
        int py2 = mouseY + (int)(sin(a_next) * radius);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            int px = mouseX + (int)(cos(a) * innerRadius);
            int py = mouseY + (int)(sin(a) * innerRadius);
